...
public class WorkSheet {
	...
	// For simplicity the table data is just stored as a hashmap from a cell's index to the cells data. 
	// Cells that are not yet part of this mapping are assumed empty.
	// Once a cell is constructed the object is not replaced in this mapping, rather the data of the cell can be modified.
	...
	public Cell lookup(CellIndex index) {
		...
		if (null ==cell) { // if the cell is not there then create it and add
							// it to the mapping
			cell = new Cell();
	...
	public void save(File file) {
		...
		for (CellIndex i : tabledata.keySet()) {
			if (!tabledata.get(i).isEmpty()) {
				...
				sf.addText(BACKGROUND, Integer.toString(tabledata.get(i).getBackground().getRGB()));
			}
		}
	...
	public static WorkSheet load(File file) {
		...
		while ((name = lf.nextElement()) != null) {
			if (name.equals(INDEXED_CELL)) {
				...
				String color = lf.getText(BACKGROUND);
				worksheet.put(index, text, color);
	...
	private void put(String index, String text, String background) {
		tabledata.put(new CellIndex(index), new Cell(text,background));
	}
}
